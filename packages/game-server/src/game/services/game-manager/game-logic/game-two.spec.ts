...
describe("game 2", () => {
  ...
  const generateListOfPlayerIds: (n: number) => PlayerModel.Id[] = (
    numOfIds
  ) => {
  ...
  const buildAction: (
    tokenDist: GameTwoModel.TokenDistribution
  ) => GameTwoModel.Turn = (tokenDist: GameTwoModel.TokenDistribution) => ({
    type: "game-two_turn",
    round: 0,
  ...
  const buildRandomTokenDistribution: () => GameTwoModel.TokenDistribution =
    () => {
      ...
      let competeTokens: number = Math.floor(
        Math.random() * (remainingTokens + 1)
      );
  ...
  const areEquivalentActions: (
    actionOne: GameTwoModel.Turn,
    actionTwo: GameTwoModel.TokenDistribution
  ) => boolean = (
    actionOne: GameTwoModel.Turn,
    actionTwo: GameTwoModel.TokenDistribution
  ) => {
  ...
  const verifyTeamResults = (
    game: GameTwo,
    listOfActions: GameTwoModel.Turn[],
    competeCoeff: number,
    investCoeff: number
  ): boolean => {
    let winningTeamCompeteTotal =
      listOfActions[0].tokenDistribution.compete +
      listOfActions[1].tokenDistribution.compete +
      listOfActions[2].tokenDistribution.compete;
    let winningTeamKeepTotal =
      listOfActions[0].tokenDistribution.keep +
      listOfActions[1].tokenDistribution.keep +
      listOfActions[2].tokenDistribution.keep;
    let winningTeamInvestTotal =
      listOfActions[0].tokenDistribution.invest +
      listOfActions[1].tokenDistribution.invest +
      listOfActions[2].tokenDistribution.invest;
    let losingTeamCompeteTotal =
      listOfActions[3].tokenDistribution.compete +
      listOfActions[4].tokenDistribution.compete +
      listOfActions[5].tokenDistribution.compete;
    let losingTeamKeepTotal =
      listOfActions[3].tokenDistribution.keep +
      listOfActions[4].tokenDistribution.keep +
      listOfActions[5].tokenDistribution.keep;
    let losingTeamInvestTotal =
      listOfActions[3].tokenDistribution.invest +
      listOfActions[4].tokenDistribution.invest +
      listOfActions[5].tokenDistribution.invest;

    // Check compete total
    for (let i = 0; i < PLAYERS_PER_GAME; i++) {
      if (i < 3) {
        if (
          game.getState(winners[i]).teamResults?.winnerTeam.competePenalty !=
          losingTeamCompeteTotal * competeCoeff
        ) {
          return false;
        }
      } else {
        if (
          game.getState(losers[i - 3]).teamResults?.loserTeam.competePenalty !=
          winningTeamCompeteTotal * competeCoeff
        ) {
          return false;
        }
      }
    ...
    for (let i = 0; i < PLAYERS_PER_GAME; i++) {
      if (i < 3) {
        if (
          game.getState(winners[i]).teamResults?.winnerTeam
            .totalTokenDistribution.keep != winningTeamKeepTotal
        ) {
          console.log("c");
          return false;
        }
        ...
        if (
          game.getState(losers[i - 3]).teamResults?.loserTeam
            .totalTokenDistribution.keep != losingTeamKeepTotal
        ) {
          console.log("d");
          return false;
        }
    ...
    for (let i = 0; i < PLAYERS_PER_GAME; i++) {
      if (i < 3) {
        if (
          game.getState(winners[i]).teamResults?.winnerTeam.investBonus !=
          winningTeamInvestTotal * investCoeff
        ) {
          console.log("e");
          return false;
        }
        ...
        if (
          game.getState(losers[i - 3]).teamResults?.loserTeam.investBonus !=
          losingTeamInvestTotal * investCoeff
        ) {
          console.log("f");
          return false;
        }
  ...
  const verifyPlayerResults = (
    ...
    for (let i = 0; i < PLAYERS_PER_GAME; i++) {
      if (i < 3) {
        if (
          listOfActions[i].tokenDistribution.keep +
            winningTeamTotal!.investBonus +
            losingTeamTotal!.competePenalty -
            winningTeamTotal!.competePenalty !=
          game.getState(winners[i]).playerResults?.netTokens
        ) {
          return false;
        }
      } else {
        if (
          listOfActions[i].tokenDistribution.keep +
            losingTeamTotal!.investBonus +
            winningTeamTotal!.competePenalty -
            losingTeamTotal!.competePenalty !=
          game.getState(losers[i - 3]).playerResults?.netTokens
        ) {
          return false;
        }
      }
  ...
  describe("bot actions", () => {
    it("makes player movements when no action is submitted", () => {
      ...
      expect(gameTwo.playerResults?.size).not.toEqual(0);
    });
  ...
  describe("submitting actions", () => {
    it("properly handles submit action", () => {
      let actionOne: GameTwoModel.Turn = buildAction(
        buildRandomTokenDistribution()
      );
      let actionTwo: GameTwoModel.Turn = buildAction(
        buildRandomTokenDistribution()
      );
      let actionThree: GameTwoModel.Turn = buildAction(
        buildRandomTokenDistribution()
      );
      let actionFour: GameTwoModel.Turn = buildAction(
        buildRandomTokenDistribution()
      );
      let actionFive: GameTwoModel.Turn = buildAction(
        buildRandomTokenDistribution()
      );
      let actionSix: GameTwoModel.Turn = buildAction(
        buildRandomTokenDistribution()
      );
  ...
  describe("generating results", () => {
    it("generates correct teamResults", () => {
      ...
      let roundZeroCompeteCoeff = gameTwo.getState(
        winners[0]
      ).competeCoefficient;
      ...
      expect(
        verifyTeamResults(
          gameTwo,
          actionList,
          roundZeroCompeteCoeff,
          roundZeroInvestCoeff
        )
      ).toEqual(true);
    });
    ...
    it("pushes to the database", () => {
      ...
      expect(game.pushToDatabase).toHaveBeenCalledWith(
        gameTwo.getSelections(),
        gameTwo.state.teamResults,
        gameTwo.receiptRoundNumber
      );
    });
  });
});
