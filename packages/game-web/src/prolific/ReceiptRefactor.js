...
const GAME_TWO_PREFIX = "Game Two";
...
function Receipt(props) {
    ...
    console.log("receipt game state: ", props.currentState);

    return (
        <div>
            {getGameOne(props.isGameOneWinner, props.gameOneAmount, classes, props.windowHeight)}
            ...
            {getGameTwoNetResults(props.currentState.playerResults.netTokens, props.currentState.playerResults.netMoney, INVEST_DELAY, classes, props.windowHeight)}
...
function getGameTwoNetResults(netTokens, netMoney, delay, classes, windowHeight) {
    let marginTop = getGameTwoResourceMargin(windowHeight);
    console.log("net money: ", netMoney);
    ...
    return (
        <div>
            ...
            <FadeIn delay={delay + AMOUNT_DELAY}>
                <div className={classes.resourceAmount}>
                    <Typography style={{ fontSize: ITEM_FONT_SIZE, color: getColor(netMoney) }} variant={"h3"}>
                        {getSign(netMoney) + netMoney.toFixed(DECIMAL_PLACES)}
                    </Typography>
                </div>
            </FadeIn>
        </div>
