...
import GameTimer from "../../common/common_components/GameTimer";
...
function ColumnController(props) {
  const [fromHeights, setFromHeights] = useState(
    createPlayerArray(BOTTOM_OF_SCREEN)
  );
  const [toHeights, setToHeights] = useState(
    createPlayerArray(scaleHeight(INITIAL_HEIGHT))
  );
  ...
  const [tooManySelections, setTooManySelections] = useState(
    NOT_TOO_MANY_SELECTIONS
  );
  const [resetTimer, setResetTimer] = useState(DO_NOT_RESET_TIMER);
  const [submitDecisions, setSubmitDecisions] = useState(
    DO_NOT_SUBMIT_DECISIONS
  );
  ...
  useEffect(() => {
    socket.on(
      ...
      (
        ...
        clearBonusArray(
          setTriples,
          tripleBonuses.length * PAUSE_BETWEEN_ANIMATIONS
        );
    ...
    socket.on("end game 1", (winners, losers, doubleBonuses, tripleBonuses) => {
      ...
      setTimeout(
        () => moveToSummary(props),
        (doubleBonuses + tripleBonuses + NORMAL_ANIMATION_OFFSET) *
          PAUSE_BETWEEN_ANIMATIONS
      );
    });
...
function handleDoubleBonuses(
  ...
  for (let i = 0; i < doubleArray.length; i++) {
    ...
    updateHeightsDelayed(
      ...
      setNewHeights,
      setCurrentHeight,
      (i + animationOffset) * PAUSE_BETWEEN_ANIMATIONS
    );
    markDoubleDelayed(
      firstIndex,
      secondIndex,
